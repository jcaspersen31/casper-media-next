...
export async function POST(request) {
  ...
  const existing = db.prepare("SELECT id FROM report_templates WHERE usage_id = ?").get(Number(usage_id));
  if (existing) {
  ...
  const row = db.prepare("SELECT * FROM report_templates WHERE id = ?").get(info.lastInsertRowid);
