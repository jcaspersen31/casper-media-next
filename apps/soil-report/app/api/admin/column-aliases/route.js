...
export async function GET() {
  ...
  const rows = await db.prepare("SELECT * FROM column_aliases ORDER BY confirmed, metric_key, header_text").all();
...
export async function POST(request) {
  ...
  const row = await db.prepare("SELECT * FROM column_aliases WHERE id = ?").get(info.lastInsertRowid);
